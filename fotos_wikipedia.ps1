# Script para buscar fotos dos 416 jogadores via Wikipedia
# Rate limit: máximo 10 requisições por 2 segundos

param(
    [string]$OutputFile = "C:\Users\Notebook\Documents\projects\bolao-copa-2026\fotos_wikipedia.json"
)

# Elencos dos 16 times da Copa 2026
$ELENCOS = @{
    'Brasil' = @{
...
# Montar lista de todos os jogadores
$jogadores = @()
foreach ($time in $ELENCOS.Keys) {
    foreach ($pos in @('GK', 'DEF', 'MID', 'FWD')) {
        foreach ($nome in $ELENCOS[$time][$pos]) {
            $jogadores += @{ nome = $nome; time = $time }
        }
    }
}
...
# Função para buscar no Wikipedia
function Get-WikipediaPhoto {
    param([string]$PlayerName, [ref]$ReqCount, [ref]$BatchTime)

    $baseUrl = "https://en.wikipedia.org/api/rest_v1/page/summary"
    $terms = @($PlayerName, ($PlayerName -split ' ')[-1])

    foreach ($term in $terms | Select-Object -Unique) {
        try {
            # Rate limit
            $ReqCount.Value++
            if ($ReqCount.Value -ge 10) {
                $elapsed = (Get-Date) - $BatchTime.Value
                if ($elapsed.TotalMilliseconds -lt 2000) {
                    Start-Sleep -Milliseconds (2000 - [int]$elapsed.TotalMilliseconds)
                }
                $ReqCount.Value = 0
                $BatchTime.Value = Get-Date
            }

            $url = "$baseUrl/$([System.Uri]::EscapeDataString($term))"
...
foreach ($jogador in $jogadores) {
    $processados++
    $pct = [int](($processados / $jogadores.Count) * 100)
    Write-Host -NoNewline ("`r[{0}%] {1}" -f $pct, $jogador.nome.PadRight(30))

    $foto = Get-WikipediaPhoto -PlayerName $jogador.nome -ReqCount ([ref]$requestCount) -BatchTime ([ref]$lastBatchTime)

    if ($foto) {
        $resultado[$jogador.nome] = $foto
        $comFoto++
    }
}
...
Write-Host ""
Write-Host "=========================================="
Write-Host ("Total de jogadores: {0}" -f $jogadores.Count)
Write-Host ("Com foto: {0} ({1}%)" -f $comFoto, [int](($comFoto / $jogadores.Count) * 100))
Write-Host ("Resultado: {0}" -f $OutputFile)
Write-Host "=========================================="
